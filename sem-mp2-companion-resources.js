(()=>{
 ...
 semMp2Lessons.forEach(lesson=>generatedTypes.forEach(type=>{
  const resource={id:`${lesson.id.toLowerCase()}-${type.key}`,title:`${lesson.id} ${type.label}`,course:lesson.course,type:type.label,lessonIds:[lesson.id],status:"Generated",verification:"Generated from the complete SEM MP2 lesson package",reviewRequired:type.reviewRequired};
  if(!existingIds.has(resource.id)){resources.push(resource);generated.push(resource);existingIds.add(resource.id);}
 }));
 const rolePlay=sources.verifiedSources.find(resource=>resource.fileId==="1m64puXnE5kv9OEWnnkqIJoM2zgPa5QEpOEygvnGCCUk");
 if(rolePlay)["SEM-035","SEM-039"].forEach(id=>{if(!rolePlay.lessonIds.includes(id))rolePlay.lessonIds.push(id);});
 const verified=[
 ...
 const demandType=material=>{const value=String(material).toLowerCase();if(value.includes("answer key"))return "Answer Key";if(value.includes("rubric")||value.includes("checklist"))return "Scoring Rubric";if(value.includes("assessment")||value.includes("performance check")||value.includes("review"))return "Assessment";return "Student Organizer";};
