...
const resourceDefaults={health:70,status:"Needs Review",version:"Unknown",lastUsed:"Not recorded",companions:[],tags:[],lessonIds:[]};
function normalizeResource(resource){
 const metadata=resourceMetadata[resource.title]||{};
 Object.entries({...resourceDefaults,...metadata}).forEach(([key,value])=>{
  if(resource[key]===undefined||resource[key]===null)resource[key]=Array.isArray(value)?[...value]:value;
 });
 if(!Array.isArray(resource.lessonIds))resource.lessonIds=[];
 if(!Array.isArray(resource.tags))resource.tags=[];
...
resourceLibrary=function(){
  ...
  const courses=["All",...new Set(resources.map(r=>r.course).filter(Boolean))];
  ...
  }).sort((a,b)=>b.health-a.health||String(a.title).localeCompare(String(b.title)));
