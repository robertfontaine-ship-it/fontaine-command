...
const scripts=[
  ...
  'fashion-lesson-packages.js','fashion-mp1-quality-patch.js','entrepreneurship-lesson-packages.js','entrepreneurship-mp1-quality-patch.js','mp1-priority-quality-patch.js',
...
test('repaired Fashion and Entrepreneurship MP1 lessons are strong',()=>{
  const ids=['ENT-001','ENT-002','ENT-011','ENT-012','ENT-013','ENT-014','ENT-018','FASH-001','FASH-002','FASH-004','FASH-007','FASH-013','FASH-014','FASH-017','FASH-018'];
...
test('audit score is a useful revision baseline',()=>{
  assert.ok(audit.average>=93&&audit.average<=100,`average ${audit.average}`);
...
test('quality dashboard loads after all curriculum packages and before usability wrapper',()=>{
  ...
  assert.ok(position('mp1-priority-quality-patch.js')<position('entrepreneurship-migration-audit.js'));
