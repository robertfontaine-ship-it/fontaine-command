...
const scripts=[
  ...
  'fashion-lesson-packages.js','fashion-mp1-quality-patch.js','mp1-fashion-final-quality-patch.js','fashion-mp1-minor-polish-quality-patch.js','entrepreneurship-lesson-packages.js','entrepreneurship-mp1-quality-patch.js','mp1-priority-quality-patch.js','mp1-ideas-color-style-quality-patch.js','mp1-trends-promotion-quality-patch.js','mp1-entrepreneurship-research-ownership-quality-patch.js','entrepreneurship-mp1-minor-polish-quality-patch.js',
...
test('repaired Fashion and Entrepreneurship MP1 lessons are strong',()=>{
  const ids=['ENT-001','ENT-002','ENT-003','ENT-004','ENT-005','ENT-006','ENT-007','ENT-008','ENT-009','ENT-010','ENT-011','ENT-012','ENT-013','ENT-014','ENT-015','ENT-016','ENT-017','ENT-018','ENT-019','ENT-020','FASH-001','FASH-002','FASH-003','FASH-004','FASH-005','FASH-006','FASH-007','FASH-008','FASH-009','FASH-010','FASH-011','FASH-012','FASH-013','FASH-014','FASH-015','FASH-016','FASH-017','FASH-018','FASH-019','FASH-020'];
...
test('eighth repair cluster clears its original quality flags',()=>{
  const originalFlags=['thin-mini-lesson','thin-activity','activity-criteria','low-rigor-exit-ticket'];
  ['FASH-005','FASH-008','FASH-015','FASH-016','FASH-019','FASH-020'].forEach(id=>{
    const result=audit.results.find(item=>item.lessonId===id);
    assert.ok(result.score>=90,`${id} scored ${result.score}`);
    assert.equal(result.issues.some(issue=>originalFlags.includes(issue.code)),false,`${id} retained an original flag`);
  });
});
test('first Minor Polish cluster clears its original quality flags',()=>{
  const originalFlags=['thin-mini-lesson','thin-activity','low-rigor-exit-ticket'];
  ['ENT-008','FASH-003','FASH-010'].forEach(id=>{
    const result=audit.results.find(item=>item.lessonId===id);
    assert.ok(result.score>=90,`${id} scored ${result.score}`);
    assert.equal(result.band,'Strong',id);
    assert.equal(result.issues.some(issue=>originalFlags.includes(issue.code)),false,`${id} retained an original flag`);
...
test('third Minor Polish cluster clears its original quality flags',()=>{
  const originalFlags=['thin-mini-lesson','thin-activity','activity-criteria'];
  ['ENT-003','FASH-006'].forEach(id=>{
    const result=audit.results.find(item=>item.lessonId===id);
    assert.ok(result.score>=90,`${id} scored ${result.score}`);
    assert.equal(result.band,'Strong',id);
    assert.equal(result.issues.some(issue=>originalFlags.includes(issue.code)),false,`${id} retained an original flag`);
...
test('all released lessons are at least Minor Polish',()=>{
  ...
  assert.ok(audit.strong>=160,`strong ${audit.strong}`);
  assert.ok(audit.minor<=20,`minor ${audit.minor}`);
...
test('quality dashboard loads after all curriculum packages and before usability wrapper',()=>{
  ...
  assert.ok(position('fashion-mp1-quality-patch.js')<position('mp1-fashion-final-quality-patch.js'));
  assert.ok(position('mp1-fashion-final-quality-patch.js')<position('fashion-mp1-minor-polish-quality-patch.js'));
  assert.ok(position('fashion-mp1-minor-polish-quality-patch.js')<position('fashion-migration-audit.js'));
  ...
  assert.ok(position('mp1-entrepreneurship-research-ownership-quality-patch.js')<position('entrepreneurship-mp1-minor-polish-quality-patch.js'));
  assert.ok(position('entrepreneurship-mp1-minor-polish-quality-patch.js')<position('entrepreneurship-migration-audit.js'));
